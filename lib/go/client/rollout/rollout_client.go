...
// New creates a new rollout API client.
func New(transport runtime.ClientTransport, formats strfmt.Registry) *Client {
	return &Client{transport: transport, formats: formats}
}

/*
Client for rollout API
*/
type Client struct {
...
func (a *Client) CreateRolloutFeatureFlag(params *CreateRolloutFeatureFlagParams) (*CreateRolloutFeatureFlagOK, error) {
	...
	success, ok := result.(*CreateRolloutFeatureFlagOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for CreateRolloutFeatureFlag: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
